...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser("complete-step", help="Mark a step as complete")
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        set_step_status(data, args.step, "complete")
    ...
    if args.as_json:
        print(json.dumps({"project": args.project, "step": args.step, "status": "complete"}))
    else:
        print(f"Step {args.step} → complete for {args.project!r}.")
    return 0
