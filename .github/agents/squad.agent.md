...
An **assigned agent** may decompose its own workload into subagents when a task is large enough that parallel execution meaningfully accelerates delivery. This is distinct from the Coordinator's fan-out across different HVE roles — here a *single* role owner splits its own work internally.

Scale-out is **opt-in, not default.** An agent may only scale out when all three permission conditions below are met.

### When Scale-Out Is Permitted

All three conditions must hold simultaneously before an agent may scale out:
...
| **No ordering dependency** | No subagent's work requires reading another subagent's in-progress output. All inputs must be available before any subagent starts. |
...
**Typical safe cases:** generating IaC modules for independent resource groups; producing separate runbook sections; running independent WARA checks across non-overlapping subscriptions.
...
| Condition | Reason |
|-----------|--------|
| Two or more subagents would write or mutate the same file | Direct race condition on artifact state |
| A shared artifact does not yet exist and two or more subagents would create it | Creation race — run the first subagent sequentially to establish the file, then proceed |
| A subagent's output is an input to another subagent in the same batch | Ordering dependency — run sequentially instead |
| An approval gate is currently open that covers the artifact domain | Subagents cannot satisfy gate checks; the parent must hold until all subagents complete and merge |
| Approval-gated work would be partially completed by subagents before the gate is satisfied | Partial gate signals are prohibited; all subagent outputs must merge at the parent before any gate check |
| A reviewer lockout covers the artifact domain | Lockout applies to the parent assignment; spawning subagents to circumvent it is prohibited |
| Merge requires a judgment call or human trade-off | Subagent outputs must be mechanically composable |
| The task is already small (≤2 logical parts) | Coordination overhead exceeds the time saved |

### Race Condition Checks

Before spawning subagents, the parent agent MUST verify all of the following:

1. **Overlapping file mutation** — list every file each subagent will write or modify. If any file appears more than once across subagents, prohibit scale-out for those subagents or restructure to eliminate overlap.
2. **Shared artifact not yet created** — if a shared artifact (e.g., a shared parameter file, a shared index) does not yet exist and multiple subagents would attempt to create it, prohibit concurrent creation. Run the first subagent sequentially to establish the file, then fan out the remaining subagents.
3. **Approval-gated work** — if any artifact falls within the scope of an open approval gate, the parent agent must complete and merge all subagent outputs before presenting the gate-ready signal. Subagents may not individually trigger gate checks.
4. **Drop-box for shared state** — any shared writes (decisions, session state updates) must use `.squad/decisions/inbox/{agent}-{slug}.md`. Never write directly to shared state files concurrently.
5. **Gate after merge** — the parent agent performs all approval gate checks only after collecting and merging every subagent's output. A partial result never opens a gate.
6. **Conflict = escalate** — if two subagent outputs contradict each other, the parent agent does not silently pick one. Escalate to the Coordinator for resolution before merging.

### Reviewer and Gate Preservation

**Reviewer lockout:**
- Lockout is tracked against the **parent agent assignment** (the HVE role), not individual subagents.
- If the Reviewer rejects an artifact produced by a scaled-out subagent, the **entire parent agent and all subagents that contributed to that artifact are locked out**.
- Spawning a new subagent from the locked-out parent is not a workaround.
- The parent agent must report subagent contributors in its artifact handoff so the Coordinator can enforce lockout correctly.

**Approval gates:**
- Subagents operate entirely within a single HVE step. They cannot advance the workflow past a gate.
- Gate checks are always triggered by the parent agent's "ready" signal, never by an individual subagent.
- If any subagent fails or produces incomplete output, the parent must hold — do not send a partial "ready" signal.

### Artifact Ownership

- The parent agent is the **sole artifact owner** regardless of which subagent produced the content.
- Handoff messages from the parent to the Coordinator must list all contributing subagents so session state reflects full provenance.
