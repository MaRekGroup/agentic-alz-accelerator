...
description: "Azure SQL Database and Managed Instance architecture, failover groups, Entra-only authentication migration, TDE with customer-managed keys, and brownfield security hardening for S3 Regulated Financial Services landing zones. USE FOR: SQL Database and Managed Instance topology design (General Purpose / Business Critical / Hyperscale tiers), failover group configuration with auto-failover policy and geo-replication, Entra-only auth migration from SQL auth (one-way enforcement), TDE with BYOK and Key Vault CMK lifecycle, private endpoint network isolation, Azure Policy for SQL compliance (deny public access, require Entra-only, require TDE CMK), Managed Instance VNet-native lift-and-shift, long-term backup retention, and brownfield S3 Regulated Financial Services SQL hardening playbook. DO NOT USE FOR: Azure Database for PostgreSQL/MySQL/MariaDB (future wave), Synapse Analytics or Fabric or Data Factory or Purview (analytics wave), Cosmos DB document and globally-distributed workloads (use azure-cosmos-db), blob/queue/table/file storage (use azure-storage-accounts), CMK key lifecycle and rotation (use azure-key-vault), private endpoint patterns (use azure-private-link), Policy authoring (use azure-policy), or SQL-vs-Cosmos-vs-Storage tier selection criteria (see docs/decisions/data-tier-selection.md)."
