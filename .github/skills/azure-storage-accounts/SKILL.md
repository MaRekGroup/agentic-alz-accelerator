...
description: "Azure Storage Accounts for multi-tenant blob isolation, lifecycle, immutability, and security hardening in Azure Landing Zones. USE FOR: standard storage workloads (Blob/Queue/Table/Files SMB/Static Website), multi-tenant isolation (account/container/path-per-tenant), Hot/Cool/Cold/Archive lifecycle, immutability for compliance (FINRA, SEC 17a-4), private endpoint + storage firewall, disabling shared-key auth in favor of Entra RBAC and user-delegation SAS, CMK encryption, blob inventory, and brownfield public-access remediation. DO NOT USE FOR: HNS/ADLS Gen2 analytics accounts (future), Azure NetApp Files, Azure Files Premium, Managed Disks, Synapse/Fabric/Data Factory/Purview, Cache for Redis, AI Search (all future), relational OLTP (use azure-sql-database), globally distributed documents (use azure-cosmos-db), or cross-cutting identity/network/cost/policy/key (use dedicated skills). Tier selection: see docs/decisions/data-tier-selection.md."
