...
**2026-05-18 — Linus — W4 session close** — Wave 4 (Data Platform tier: SQL/Cosmos/Storage) shipped via PR #69; decision ADR and 3 SKILL.md files with 3 HARD GATEs each merged to main. Skill capacity 12/14 complete. Next: W5 hybrid skills.

## 2026-05-19 — Wave 5 (Hybrid)
- Linus-7 (claude-sonnet-4.6): Authored plan `.squad/decisions/inbox/linus-wave5-plan.md` (~320 lines, 9/9 self-grade PASS). Confirmed scope: `azure-arc-servers` (S6, CAF Management/Governance/Security, WAF OpsEx primary) + `azure-arc-kubernetes` (S8, CAF Platform Automation/Management/Governance, WAF OpsEx primary). ADR: YES (`docs/decisions/hybrid-onboarding-strategy.md`), 9 sections, Phase 1A sequential before Saul fan-out. Key trade-offs: (1) Rejected arc-data-services as a 3rd skill — no scenario anchor, would fragment Arc K8s; (2) Arc K8s hard prereq on AKS skill (Flux v2 GitOps reference); (3) Both skills carry ⛔ HARD GATEs for irreversible mutations (SP credential scope for arc-servers; OIDC issuer enable for arc-kubernetes). Description drafts sized ≤1020 chars (2026-05-19 hotfix constraint baked in). 5 Open Questions for Yeselam: Arc data services catalog finality, scenario code assignment (S6/S8 vs S9/S10), onboarding credential pattern recommendation, brownfield vs balanced weighting, and AGENTS.md section placement (Networking vs new Hybrid section). Open item for Isabel: validate that the Arc agent helm install HARD GATE (step 3, arc-kubernetes) is positioned correctly as pre-mutation — it's the most operationally risky mutation in the W5 playbooks.
- Linus-8 (claude-sonnet-4.6): Authored shared ADR `docs/decisions/hybrid-onboarding-strategy.md` (129 lines, 9 sections). Baked all 5 Yeselam-resolved decisions: Arc data services out-of-scope stated in §2 Context; S6/S8 scenario codes in §8; MI-first credential default explicit at line 51 (§5 Onboarding Sequence Model); balanced greenfield+brownfield framing throughout; downstream Hybrid AGENTS.md section noted in §9 References. Five anti-patterns: vanity Arc onboarding, credential proliferation, policy fragmentation, agent drift, governance bypass via exemptions. Phase 2 (Saul-arc-servers + Saul-arc-kubernetes parallel fan-out) is now unblocked.
